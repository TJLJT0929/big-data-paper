...
\vspace{0.5cm}
	...
	对因变量\texttt{loan\_status}的频率统计显示，全额偿还（Fully Paid）样本占总体的80.38\%，违约核销（Charged Off）样本占19.62\%。图~\ref{fig:loan_status}直观展示了这一类别分布情况。可以看出，非违约样本数量远超违约样本，二者之比约为4:1，呈现出信用风险建模中常见的类别不均衡特征。这一分布特点意味着，如果模型简单地将所有样本预测为多数类（全额偿还），也能获得约80\%的准确率，因此在后续建模过程中需引入更具区分力的评价指标（如ROC-AUC），并充分考虑类别不均衡对模型训练的影响。
	
	\begin{figure}[htbp]
		\centering
		\includegraphics[width=0.6\textwidth]{loan_status_distribution.png}
		\caption{贷款状态（Fully Paid与Charged Off）分布柱状图}
		\label{fig:loan_status}
	\end{figure}
	...
	\begin{figure}[htbp]
		\centering
		\includegraphics[width=0.85\textwidth]{loan_amnt_installment.png}
		\caption{贷款金额与月还款额按贷款状态的分布（直方图与箱线图）}
		\label{fig:loan_amnt_installment}
	\end{figure}
	
	\subsubsection{贷款等级与子等级分析}
	
	LendingClub依据借款人信用状况将贷款划分为A（最优）至G（最差）七个等级。图~\ref{fig:grade_subgrade}展示了按等级和子等级划分的贷款状态分布，统计结果揭示了清晰的违约梯度规律：从A至G等级，违约率呈单调递增态势。A等级贷款的违约占比最低，而F、G等级贷款的违约占比最高。图~\ref{fig:fg_grade}进一步聚焦于F、G子等级内部的分布情况，可以发现违约样本与全额偿还样本的数量差距明显收窄，部分子等级的违约率已接近50\%。上述结果表明，LendingClub的内部评级体系对借款人违约风险具有较好的区分效力，评级信息对信用风险建模具有重要的参考价值。
	
	\begin{figure}[htbp]
		\centering
		\includegraphics[width=0.85\textwidth]{grade_subgrade_distribution.png}
	...
	\begin{figure}[htbp]
		\centering
		\includegraphics[width=0.85\textwidth]{fg_grade_default.png}
	...
	\begin{figure}[htbp]
		\centering
		\includegraphics[width=0.85\textwidth]{term_home_verify_purpose.png}
	...
	\begin{figure}[htbp]
		\centering
		\includegraphics[width=0.85\textwidth]{int_rate_distribution.png}
	...
	\begin{figure}[htbp]
		\centering
		\includegraphics[width=0.85\textwidth]{annual_inc_distribution.png}
	...
	\begin{figure}[htbp]
		\centering
		\includegraphics[width=0.85\textwidth]{dti_revol_distribution.png}
		\caption{债务收入比与循环信用利用率按贷款状态的分布}
		\label{fig:dti_revol}
	\end{figure}
	...
	\begin{figure}[htbp]
		\centering
		\includegraphics[width=0.85\textwidth]{pub_rec_bankruptcy.png}
	...
	\begin{figure}[htbp]
		\centering
		\includegraphics[width=0.75\textwidth]{feature_correlation_with_target.png}
	...
	\begin{figure}[htbp]
		\centering
		\includegraphics[width=0.85\textwidth]{ann_training_curves.png}
		\caption{ANN模型训练过程中的损失函数值与AUC指标演化曲线}
		\label{fig:ann_training}
	\end{figure}
	
	ANN模型的训练集与测试集准确率十分接近（88.84\%与88.87\%），未表现出明显的过拟合迹象。图~\ref{fig:ann_training}展示了ANN模型在训练过程中损失函数值和AUC指标随轮次的演化曲线，可以观察到训练损失在前几个轮次内迅速下降并趋于平稳，AUC指标则同步上升并收敛，说明模型的学习过程稳定且有效。从分类报告来看，该模型对全额偿还类的识别能力突出（召回率达0.99），但对违约类的识别能力偏弱（召回率仅0.46），这在一定程度上反映了样本类别不均衡带来的影响——模型在优化过程中倾向于偏重多数类以降低总体损失。ANN模型的ROC-AUC得分为0.905，在三种模型中居于首位，表明其在正负样本综合判别方面具有较强的能力。
	...
	训练集混淆矩阵显示，实际违约中被正确识别25{,}828条、误判25{,}837条，实际偿还中被误判1{,}423条、正确识别209{,}055条。图~\ref{fig:xgb_cm}展示了XGBoost模型在测试集上的混淆矩阵，从中可以直观地看到各类别的预测准确情况。
	
	\begin{figure}[htbp]
		\centering
		\includegraphics[width=0.6\textwidth]{xgboost_confusion_matrix.png}
		\caption{XGBoost模型在测试集上的混淆矩阵}
		\label{fig:xgb_cm}
	\end{figure}
	
	XGBoost模型的训练准确率（89.60\%）略高于测试准确率（88.94\%），两者差距较小，泛化能力尚可。违约类召回率（0.48--0.50）略优于ANN模型，意味着XGBoost在违约样本的捕获能力上具有一定的优势，但其ROC-AUC得分仅为0.734，低于ANN，说明其对正负样本的综合判别能力不及神经网络模型。
	...
	训练集混淆矩阵显示，实际违约51{,}665条全部被正确识别，实际偿还210{,}478条全部被正确识别，呈现出完美分类的结果。然而，这一训练集上的"零误差"表现恰恰是过拟合的典型征兆。
	...
	\begin{figure}[htbp]
		\centering
		\includegraphics[width=0.6\textwidth]{rf_confusion_matrix.png}
		\caption{随机森林模型在测试集上的混淆矩阵}
		\label{fig:rf_cm}
	\end{figure}
	
	随机森林在训练集上达到了100\%的准确率，但测试集仅为88.94\%，两者之间存在显著落差，表明该模型出现了较为严重的过拟合。图~\ref{fig:rf_cm}展示了随机森林模型在测试集上的混淆矩阵，可以清晰地看到其在违约类上的误判情况。随机森林的ROC-AUC得分为0.725，在三种模型中最低。这一结果说明，在未对树深度和叶节点最小样本数等超参数进行有效约束的情况下，随机森林容易在高维特征空间中产生过拟合，导致泛化性能受损。
	
	\subsubsection{模型综合比较}
	
	三种模型在测试集上的核心指标汇总于表~\ref{tab:model_compare}，图~\ref{fig:model_compare}以柱状图的形式直观对比了三种模型在训练集与测试集上的ROC-AUC得分差异。
