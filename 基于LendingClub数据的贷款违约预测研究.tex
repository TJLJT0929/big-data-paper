...
\pagestyle{fancy}
\fancyhead[L]{}
\fancyhead[C]{\fangsong 基于层次聚类的多模态多目标优化问题的差分进化算法}
\fancyhead[R]{}
...
\vspace{0.5cm}
 ==================== 正文部分（不含导言区） ====================
	...
	\textbf{关键词：}P2P网络借贷；贷款违约预测；LendingClub；机器学习；探索性数据分析；人工神经网络；XGBoost；随机森林
	...
	针对上述不足，本文以LendingClub平台贷款数据为实证样本，围绕以下四项目标展开研究：
	
	\begin{enumerate}
		\item 通过系统的探索性数据分析，揭示贷款违约行为的主要驱动因素及其统计分布特征；
		\item 制定规范化的数据预处理与特征工程流程，构建适用于建模的高质量样本集；
		\item 分别构建人工神经网络、XGBoost和随机森林三种预测模型，并对其分类性能进行多指标比较；
		\item 为P2P借贷平台的贷款审批决策与风险防控提供数据驱动的参考依据。
	\end{enumerate}
	
	从理论层面看，本研究通过将深度学习��型纳入P2P信用评估的比较框架，丰富了该领域的方法论体系，为后续研究中的模型选择提供了实证参照。从实践层面看，研究成果可为LendingClub等平台的风险管控与决策优化提供技术支撑和策略参考。
	...
	针对数据质量问题，实施如下操作：
	\begin{itemize}
		\item \textbf{缺失值处理：}对缺失比例较低的特征（如\texttt{revol\_util}、\texttt{pub\_rec\_bankruptcies}，缺失率$<$0.5\%），直接剔除含缺失值的样本；对缺失比例较高的特征（如\texttt{mort\_acc}），利用与其相关性最强的变量（\texttt{total\_acc}）按分组均值进行填补。
		\item \textbf{特征删除：}移除唯一值过多、建模价值有限或可能引发数据泄露的变量（\texttt{emp\_title}、\texttt{emp\_length}、\texttt{title}、\texttt{issue\_d}等）。
		\item \textbf{异常值处理：}依据探索性分析阶段的统计发现，在训练集中按规则剔除极端样本（如年收入超过250{,}000美元、DTI超过50等）。
		\item \textbf{特征编码：}对类别型变量执行独热编码（One-Hot Encoding），涵盖\texttt{sub\_grade}、\texttt{verification\_status}、\texttt{purpose}等字段。
		\item \textbf{特征转换：}将\texttt{term}由字符串映射为数值型，从\texttt{address}中提取邮政编码信息，从\texttt{earliest\_cr\_line}中提取年份。
		\item \textbf{数据标准化：}采用MinMaxScaler将全部特征归一化至$[0, 1]$区间。
	\end{itemize}
	
	\textbf{第三阶段：模型构建与训练。}
	将数据集按67:33的比例划分为训练集（264{,}796条）与测试集（130{,}423条）。经异常值剔除后，训练集保留262{,}143条记录，最终特征维度为79维。本研究构建了以下三种分类模型：
	
	\begin{enumerate}
		\item \textbf{人工神经网络（ANN）。}基于Keras框架搭建前馈神经网络，包含3个隐藏层（每层150个神经元），隐藏层激活函数采用ReLU，输出层采用Sigmoid函数。训练过程中引入批标准化（Batch Normalization）及Dropout（丢弃率0.1）以抑制过拟合。优化算法选用Adam（学习率0.001），损失函数为二元交叉熵，共训练20轮次，小批量大小设为32。
		\item \textbf{XGBoost分类器。}调用XGBoost库中的\texttt{XGBClassifier}接口，采用默认超参数配置进行训练。XGBoost是一种基于梯度提升框架的集成学习方法，通过逐轮构建决策树拟合前一轮的残差，最终将各弱学习器的输出加以集成\cite{chen2016xgboost}。
		\item \textbf{随机森林分类器。}使用Scikit-learn中的\texttt{RandomForestClassifier}，基学习器数量设为100棵。随机森林通过自助采样（Bootstrap Sampling）与随机特征子集选取构建决策树集合，并以投票方式确定最终预测类别\cite{breiman2001random}。
	\end{enumerate}
	
	\textbf{第四阶段：模型评估与比较。}
	采用如下指标体系对三种模型的分类效果进行综合评判：
	\begin{itemize}
		\item 准确率（Accuracy）：被正确分类的样本占总样本的比例；
		\item 精确率（Precision）：模型判定为正类的样本中真正为正类的比例；
		\item 召回率（Recall）：实际正类样本中被模型正确识别的比例；
		\item F1值：精确率与召回率的调和平均数；
		\item ROC-AUC：受试者工作特征曲线下面积，综合度量模型对正负样本的区分能力；
		\item 混淆矩阵：以矩阵形式直观呈现各类别的预测结果。
	\end{itemize}
	...
	对因变量\texttt{loan\_status}的频率统计显示，全额偿还（Fully Paid）样本占总体的80.38\%，违约核销（Charged Off）样本占19.62\%。这一分布呈现出信用风险建模中常见的类别不均衡特征，在后续建模过程中需予以充分考虑。
	
	%【图1：贷款状态分布柱状图】
	
	\subsubsection{特征间相关性分析}
	
	借助Pearson相关系数热力图对数值型特征间的线性关联强度进行检验，主要发现如下：
	...
	按贷款状态对贷款金额和月还款额进行分层考察，结果显示：违约样本的贷款金额中位数及均值均略高于全额偿还样本，提示较高的贷款金额可能与违约风险的上升存在关联。月还款额的分布趋势与贷款金额基本一致。
	
	%【���3：贷款金额与月还款额按贷款状态的分布图】
	
	\subsubsection{贷款等级与子等级分析}
	
	LendingClub依据借款人信用状况将贷款划分为A（最优）至G（最差）七个等级。统计结果揭示了以下规律：
	
	从A至G等级，违约率呈单调递增态势。A等级贷款的违约占比最低，而F、G等级贷款的违约占比最高。进一步观察F、G子等级内部的分布，违约样本与全额偿还样本的数量差距明显收窄，部分子等级的违约率已接近50\%。上述结果表明，LendingClub的内部评级体系对借款人违约风险具有较好的区分效力。
	...
	\textbf{贷款期限。}60个月期限贷款的违约率显著高于36个月期限贷款，说明较长的还款周期增加了违约的可能性。
	
	\textbf{住房所有权。}租房（RENT）和按揭（MORTGAGE）类借款人样本量最大。不同住房类别间的违约率差异并不显著，但``OTHER''类别因样本极少而缺乏统计稳定性。研究中将``ANY''和``NONE''两类合并至``OTHER''。
	
	\textbf{收入验证状态。}已验证（Verified）、来源已验证（Source Verified）与未验证（Not Verified）三类借款人的违约比例相近，收入验证状态对违约预测的区分度有限。
	
	\textbf{贷款用途。}债务整合（debt\_consolidation）是最为普遍的贷款用途，其次为信用卡偿还（credit\_card）。不同用途类别间的违约率存在一定差异。
	
	%【图6：贷款期限、住房状况、验证状态与贷款用途按贷款状态的分布图】
	
	\subsubsection{利率与年收入分析}
	
	\textbf{贷款利率。}违约样本的利率分布明显偏向高利率区间，表明利率水平是违约预测中的重要判别变量。
	
	\textbf{年收入。}年收入低于250{,}000美元的借款人占据了绝大多数。年收入超过1{,}000{,}000美元的借款人仅有75人（不足样本总量的0.02\%），而超过250{,}000美元的共计4{,}077人。总体而言，高收入群体的违约率相对较低。
	...
	\textbf{债务收入比（DTI）。}随着DTI值升高，借款人的违约倾向有所增强。DTI超过50的极端记录数量极少，在建模阶段按异常值予以剔除。
	
	\textbf{循环信用利用率。}较高的循环信用利用率与违约风险的提升具有正向关联。利用率超过120\%的记录极为罕见，同样按异常值处理。
	
	\textbf{循环信用余额。}余额超过250{,}000美元的样本极少，其中也包含违约记录。
	
	%【图9：DTI和循环信用利用率按贷款状态的分布图】
	
	\subsubsection{公共记录与破产记录分析}
	
	绝大多数借款人不存在不良公共记录（\texttt{pub\_rec}=0），而有不良记录的借款人违约率偏高。破产记录的分布与之类似：大部分借款人无破产历史，有破产记录者的违约风险略有抬升。在特征工程阶段，将不良记录和破产记录分别转化为二值变量（0或1），以简化特征表达。
	
	%【图10：公共记录、初始上市状态、申请类型、破产记录和抵押贷款账户按贷款状态的分布图】
	
	\subsubsection{数值特征与因变量的相关性排序}
	
	计算全部数值型特征与因变量之间的Pearson相关系数并降序排列，结果显示：与偿还行为正相关的特征中，抵押贷款账户数和年收入的正向效应较为微弱；与违约行为正相关（即与偿还行为负相关）的特征中，利率（\texttt{int\_rate}）的负相关程度最为突出，其后依次为贷款期限（\texttt{term}）和债务收入比（\texttt{dti}）。
	...
	\begin{table}[htbp]
		...
		\begin{tabular}{llll}
			...
			\texttt{revol\_util}           & 少量 & $<$0.5\% & 删除��缺失值的行 \\
	...
	经过特征工程处理后，数据集最终包含79个特征维度（含独热编码产生的虚拟变量），主要操作包括：
	
	\begin{itemize}
		\item 将\texttt{term}由字符型（`` 36 months''/`` 60 months''）映射为数值（36/60）；
		\item 删除\texttt{grade}特征，因\texttt{sub\_grade}已包含其全部信息；
		\item 对\texttt{sub\_grade}、\texttt{verification\_status}、\texttt{purpose}、\texttt{initial\_list\_status}、\texttt{application\_type}、\texttt{home\_ownership}进行独热编码（设置drop\_first=True以规避多重共线性）；
		\item 从\texttt{address}字段中提取邮政编码并进行独热编码；
		\item 删除\texttt{issue\_d}以防止数据泄露；
		\item 从\texttt{earliest\_cr\_line}中提取年份信息；
		\item 将\texttt{pub\_rec}、\texttt{mort\_acc}、\texttt{pub\_rec\_bankruptcies}转化为二值变量。
	\end{itemize}
	...
	需要指出的是，异常值剔除仅在训练集上实施，测试集保持原始分布不变，以便更客观地评价模型在真实数据环境下的泛化表现。处理后训练集由264{,}7960 条缩减至262{,}1430 条，剔除比例约为1.0\%。
	
	\subsection{模型预测结果}
	
	\subsubsection{人工神经网络模型结果}
	
	ANN模型在训练集和测试集上的分类性能分别如表~\ref{tab:ann_train}和表~\ref{tab:ann_test}所示，对应的混淆矩阵见表~\ref{tab:ann_cm_train}和表~\ref{tab:ann_cm_test}。
	
	\begin{table}[htbp]
		\centering
		\caption{ANN模型训练集分类报告}\label{tab:ann_train}
		\begin{tabular}{lccc}
			\toprule
	...
	\begin{table}[htbp]
		\centering
		\caption{ANN模型训练集混淆矩阵}\label{tab:ann_cm_train}
		\begin{tabular}{lcc}
			\toprule
			& 预测违约 & 预测偿还 \\
			\midrule
	...
	\begin{table}[htbp]
		\centering
		\caption{ANN模型测试集分类报告}\label{tab:ann_test}
		\begin{tabular}{lccc}
			\toprule
			指标 & 违约类（0） & 全额偿还类（1） & 总体准确率 \\
			\midrule
	...
	\begin{table}[htbp]
		\centering
		\caption{ANN模型测试集混淆矩阵}\label{tab:ann_cm_test}
		\begin{tabular}{lcc}
			\toprule
			& 预测违约 & 预测偿还 \\
			\midrule
	...
	ANN模型的训练集与测试集准确率十分接近（88.84\%与88.87\%），未表现出明显的过拟合迹象。该模型对全额偿还类的识别能力突出（召回率达0.99），但对违约类的识别能力偏弱（召回率仅0.46），这在一定程度上反映了样本类别不均衡带来的影响。ANN模型的ROC-AUC得分为0.905，在三种模型中居于首位。
	
	\subsubsection{XGBoost分类器结果}
	
	XGBoost模型的分类性能如表~\ref{tab:xgb_train}和表~\ref{tab:xgb_test}所示。
	
	\begin{table}[htbp]
		\centering
		\caption{XGBoost模型训练集分类报告}\label{tab:xgb_train}
		\begin{tabular}{lccc}
			\toprule
			指标 & 违约类（0） & 全额偿还类（1） & 总体准确率 \\
			\midrule
	...
	\begin{table}[htbp]
		\centering
		\caption{XGBoost模型测试集分类报告}\label{tab:xgb_test}
		\begin{tabular}{lccc}
			\toprule
			指标 & 违约类（0） & 全额偿还类（1） & 总体准确率 \\
			\midrule
			...
			召回率 & 0.48 & 0.99 & --- \\
			F1值   & 0.63 & 0.94 & --- \\
			\textbf{准确率} & --- & --- & \textbf{88.94\%} \\
			\bottomrule
		\end{tabular}
	\end{table}
	
	训练集混淆矩阵：实际违约中被正确识别25{,}828条、误判25{,}837条；实际偿还中被误判1{,}423条、正确识别209{,}055条。
	
	%【图13：XGBoost模型的混淆矩阵与ROC曲线】
	
	XGBoost模型的训练准确率（89.60\%）略高于测试准确率（88.94\%），两者差距较小，泛化能力尚可。违约类召回率（0.48--0.50）略优于ANN模型，但其ROC-AUC得分仅为0.734，低于ANN。
	
	\subsubsection{随机森林分类器结果}
	
	随机森林模型的分类性能如表~\ref{tab:rf_train}和表~\ref{tab:rf_test}所示。
	
	\begin{table}[htbp]
		\centering
		\caption{随机森林模型训练集分类报告}\label{tab:rf_train}
		\begin{tabular}{lccc}
			\toprule
			指标 & 违约类（0） & 全额偿还类（1） & 总体准确率 \\
			\midrule
			...
			召回率 & 1.00 & 1.00 & --- \\
			F1值   & 1.00 & 1.00 & --- \\
			\textbf{准确率} & --- & --- & \textbf{100.00\%} \\
			\bottomrule
		\end{tabular}
	\end{table}
	
	训练集混淆矩阵：实际违约51{,}665条全部被正确识别，实际偿还210{,}478条全部被正确识别。
	
	\begin{table}[htbp]
		\centering
		\caption{随机森林模型测试集分类报告}\label{tab:rf_test}
		\begin{tabular}{lccc}
			\toprule
			指标 & 违约类（0） & 全额偿还类（1） & 总体准确率 \\
			\midrule
	...
	随机森林在训练集上达到了100\%的准确率，但测试集仅为88.94\%，两者之间存在显著落差，表明该模型出现了较为严重的过拟合。其ROC-AUC得分为0.725，在三种模型中最低。
	
	\subsubsection{模型综合比较}
	
	三种模型在测试集上的核心指标汇总于表~\ref{tab:model_compare}。
	...
	就ROC-AUC而言，ANN以0.905的得分大幅领先于XGBoost和随机森林，说明其在正负样本的综合判别能力上占有显著优势，能够更充分地捕捉贷款数据中的非线性特征交互。
	
	就准确率而言，三种模型的测试集准确率均在89\%附近，差异甚微。但鉴于数据集中非违约样本占比高达80\%，单纯依赖准确率评判模型优劣并不充分，ROC-AUC才是更具参考价值的综合性指标。
	
	就过拟合程度而言，ANN模型的训练与测试性能几乎一致，泛化能力最优；XGBoost略有过拟合但幅度可控；随机森林训练集准确率达100\%而测试集骤降至88.94\%，过拟合问题突出，后续可通过限制树深度、增大叶节点最小样本数等手段加以缓解。
	...
	经探索性分析与相关性检验，本文识别出如下影响违约行为的核心变量：
	
	贷款利率（\texttt{int\_rate}）是最具区分力的违约预测因子。高利率贷款的违约风险显著偏高，原因在于高利率通常对应较高的信用风险等级，同时也加重了借款人的偿还负担。贷款等级（\texttt{grade}/\texttt{sub\_grade}）与违约率之间表现出高度关联，从A等级到G等级违约率单调递增，反映出平台评级体系在信用风险辨识方面的有效性。贷款期限（\texttt{term}）对违约概率有显著影响，60个月期限贷款的违约率远高于36个月期限贷款。债务收入比（\texttt{dti}）与违约风险正相关，年收入（\texttt{annual\_inc}）与违约风险负相关，循环信用利用率（\texttt{revol\_util}）较高的借款人违约风险更大。
	
	上述因素可概括为贷款属性维度（利率、等级、期限等）与借款人财务维度（收入、DTI、循环信用利用率等），二者共同构成了违约概率的决定性因素群。
	
	\textbf{（2）模型性能对比}
	
	在三种分类器中，ANN在ROC-AUC指标上显著优于XGBoost和随机森林，体现了深度神经网络在挖掘高维数据中复杂特征交互关系方面的能力优势。具体而言：ANN的测试集AUC为0.905，分别高出XGBoost和随机森林17.1与18.0个百分点；ANN的泛化能力最为稳健，训练集与测试集性能几乎无差异；随机森林模型则面临严重的过拟合问题。不过，三种模型在违约类的召回率上均不理想（约0.46--0.50），凸显了类别不均衡对模型训练的制约。
	
	\textbf{（3）类别不均衡的影响}
	
	违约样本仅占总体的19.62\%，这一失衡导致所有模型倾向于将样本判定为多数类（全额偿还），从而牺牲了对少数类（违约）的识别灵敏度。在实际风控场景中，漏放一位违约借款人所造成的经济损失远大于误拒一位优质借款人带来的业务损失，因而违约类的召回率在应用中具有更高的优先级。
	
	\subsection{讨论}
	
	\textbf{（1）模型选择}
	
	ANN在AUC指标上的优势与既有文献的结论相吻合\cite{kvamme2018predicting}。神经网络通过多层非线性映射能够自动发现特征间的高阶交互，而传统树模型在这一方面的能力相对有限。但ANN的``黑箱''属性也带来了可解释性不足的问题，在金融监管日趋严格的背景下，这一缺陷可能制约其在实际审批流程中的应用落地。
	
	此外，本研究中XGBoost和随机森林均沿用了默认或较简单的超参数配置，尚未经过系统调优。若引入网格搜索或贝叶斯优化等方法对超参数空间进行充分探索，这两种模型的性能有望获得显著提升，与ANN之间的差距也可能相应缩小。
	
	\textbf{（2）类别不均衡处理}
	
	类别不均衡是本研究中制约违约识别效果的关键因素。后续研究可考虑以下改进路径：过采样方法（如SMOTE\cite{chawla2002smote}、ADASYN等）、欠采样方法（如随机欠采样、Tomek Links等）、代价敏感学习（为不同类别赋予差异化的误分类代价权重）以及专门面向不均衡数据的集成方法（如BalancedRandomForest、EasyEnsemble等）\cite{he2009learning}。
	
	\textbf{（3）特征工程的改进空间}
	
	本文在特征工程方面已覆盖缺失值填补、异常值剔除、独热编码及归一化等环节，但仍存在优化余地。例如：可引入L1正则化或基于树模型的特征重要性进行特征筛选；可构建交互特征（如利率与贷款金额的乘积项）以增强模型表达力；可尝试目标编码（Target Encoding）等更精细的类别特征处理策略。
	
	\textbf{（4）研究局限性}
	
	本研究存在以下不足：样本仅覆盖LendingClub一家平台，结论的外推性有待在其他平台或市场上加以验证；受数据可得性限制，未引入宏观经济指标（如失业率、GDP增速等）作为外部特征；XGBoost和随机森林的超参数优化尚不充分；未对模型的特征贡献进行深入的可解释性分析（如SHAP方法\cite{lundberg2017unified}）。
	
	\subsection{未来研究方向}
	
	基于上述结论与讨论，后续工作可从以下方向拓展：
	
	\begin{enumerate}
		\item \textbf{模型调优与架构探索。}采用贝叶斯优化等方法系统搜索超参数空间，充分释放各模型的潜力；同时尝试引入注意力机制、Transformer等更先进的深度学习架构。
		\item \textbf{不均衡学习策略比较。}系统评估各类过采样、欠采样及代价敏感学习策略对违约识别效果的提升幅度。
		\item \textbf{可解释性分析。}利用SHAP等工具量化各特征对预测结果的边际贡献，增强模型的透明度与可审计性。
		\item \textbf{时序维度建模。}结合贷款发放时间序列信息，挖掘违约模式随时间的演化规律。
		\item \textbf{外部数据融合。}纳入宏观经济指标、社会网络数据等外部信息源，拓展特征空间，提升预测精度。
	\end{enumerate}
